 ...
 * from
 * {@link java.lang.ProcessBuilder} in that a pty is used to communicate with
 * the subprocess. <p> Pseudo-terminals are a powerful Unix feature, that allows
 * programs to interact with other programs they start in slightly more
 * human-like way. For example, a pty owner can send ^C (aka SIGINT) to attached
 * shell, even if said shell runs under a different user ID.
 */
public class TermExec {
  // Warning: bump the library revision, when an incompatible change happens
  static { System.loadLibrary("jackpal-termexec2"); }

  public static final String SERVICE_ACTION_V1 =
  ...
  public @NonNull List<String> command() { return command; }

  public @NonNull Map<String, String> environment() { return environment; }
   ...
   * multiplexer device (located at /dev/ptmx). <p> Callers are responsible for
   * closing the descriptor.
   *
   * @return the PID of the started process.
   */
  public int start(final @NonNull ParcelFileDescriptor ptmxFd)
      throws IOException {
  ...
  /**
   * Send signal via the "kill" system call. Android {@link
   * android.os.Process#sendSignal} does not allow negative numbers (denoting
   * process groups) to be used.
   */
  public static native void sendSignal(int processId, int signal);

  static int createSubprocess(final ParcelFileDescriptor masterFd,
                              final String cmd, final String[] args,
                              final String[] envVars) throws IOException {
    final int integerFd;
  ...
  private static native int createSubprocessInternal(String cmd, String[] args,
                                                     String[] envVars,
                                                     int masterFd);
}
