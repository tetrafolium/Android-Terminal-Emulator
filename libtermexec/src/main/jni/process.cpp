...
static int throwIOException(JNIEnv *env, int errnum, const char *message) {
	__android_log_print(ANDROID_LOG_ERROR, LOG_TAG, "%s errno %s(%d)", message,
...
static void closeNonstandardFileDescriptors() {
	// Android uses shared memory to communicate between processes. The file
	// descriptor is passed to child processes using the environment variable
	// ANDROID_PROPERTY_WORKSPACE, which is of the form
	// "properties_fd,sizeOfSharedMemory"
	int properties_fd = -1;
...
static int create_subprocess(JNIEnv *env, const char *cmd, char *const argv[],
                             char *const envp[], int masterFd) {
	...
	// Early (Android 1.6) bionic versions of ptsname_r had a bug where they
	// returned the buffer instead of 0 on success.  A compatible way of telling
	// whether ptsname_r succeeded is to zero out errno and check it after the
	// call
	errno = 0;
...
JNIEXPORT void JNICALL Java_jackpal_androidterm_TermExec_sendSignal(
	JNIEnv *env, jobject clazz, jint procId, jint signal) {
	kill(procId, signal);
}

JNIEXPORT jint JNICALL Java_jackpal_androidterm_TermExec_waitFor(JNIEnv *env,
                                                                 jclass clazz,
                                                                 jint procId) {
	int status;
...
Java_jackpal_androidterm_TermExec_createSubprocessInternal(
	JNIEnv *env, jclass clazz, jstring cmd, jobjectArray args,
	...
	if (size > 0) {
		...
		for (int i = 0; i < size; ++i) {
			jstring var =
				reinterpret_cast<jstring>(env->GetObjectArrayElement(envVars, i));
