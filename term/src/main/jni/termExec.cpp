...
static void android_os_Exec_setPtyWindowSize(JNIEnv *env, jobject clazz,
	...
	// TODO: handle the situation, when the file descriptor is incompatible with
	// TIOCSWINSZ (e.g. not from /dev/ptmx)
	if (ioctl(fd, TIOCSWINSZ, &sz) == -1)
		env->ThrowNew(env->FindClass("java/io/IOException"),
		              "Failed to issue TIOCSWINSZ ioctl");
}

// tcgetattr /tcsetattr are not part of Bionic at API level 4. Here's a
// compatible version.
...
static void android_os_Exec_setPtyUTF8Mode(JNIEnv *env, jobject clazz, jint fd,
                                           jboolean utf8Mode) {
	struct termios tios;
	...
	if (my_tcsetattr(fd, &tios) != 0)
		env->ThrowNew(env->FindClass("java/io/IOException"),
		              "Failed to set terminal UTF-8 mode");
}
