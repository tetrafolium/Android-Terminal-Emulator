...
public final class ShortcutEncryption {
  ...
  public static final class Keys {
    ...
    public String encode() {
      return encodeToBase64(encKey.getEncoded()) + ":" +
          encodeToBase64(macKey.getEncoded());
    }
    ...
    public static Keys decode(final String encodedKeys) {
      ...
      SecretKey encKey =
          new SecretKeySpec(decodeBase64(keys[0]), ENC_ALGORITHM);
      SecretKey macKey =
          new SecretKeySpec(decodeBase64(keys[1]), MAC_ALGORITHM);
   ...
   *
   * @throws GeneralSecurityException if the data is invalid, verification
   *     fails, or an error occurs during decryption.
   */
  public static String decrypt(final String encrypted, final Keys keys)
    ...
    cipher.init(Cipher.DECRYPT_MODE, keys.getEncKey(),
   ...
   */
  public static String encrypt(final String data, final Keys keys)
      throws GeneralSecurityException {
    Cipher cipher = Cipher.getInstance(ENC_SYSTEM);
    ...
    // Encrypt
    cipher.init(Cipher.ENCRYPT_MODE, keys.getEncKey(),
                new IvParameterSpec(ivBytes));
   ...
   */
  private static String computeMac(final String data, final SecretKey key)
      throws GeneralSecurityException {
  ...
  private ShortcutEncryption() { throw new UnsupportedOperationException(); }
}
