...
class GenericTermSession extends TermSession {
  ...
  GenericTermSession(final ParcelFileDescriptor mTermFd,
                     final TermSettings settings, final boolean exitOnEOF) {
    super(exitOnEOF);
   ...
   */
  void setPtyWindowSize(final int row, final int col, final int xpixel,
                        final int ypixel) {
    // If the tty goes away too quickly, this may get called after it's
    // descriptor is closed
    if (!mTermFd.getFileDescriptor().valid())
      return;

    try {
      Exec.setPtyWindowSizeInternal(getIntFd(mTermFd), row, col, xpixel,
                                    ypixel);
    } catch (IOException e) {
  ...
  /**
   * @return true, if failing to operate on file descriptor deserves an
   *     exception (never the case for ATE own shell)
   */
  boolean isFailFast() { return false; }
  ...
  private static int getIntFd(final ParcelFileDescriptor parcelFd)
      throws IOException {
