...
public class ShellTermSession extends GenericTermSession {
  ...
  public ShellTermSession(final TermSettings settings,
                          final String initialCommand) throws IOException {
    ...
    mWatcherThread = new Thread() {
      ...
      public void run() {
        ...
        mMsgHandler.sendMessage(
            mMsgHandler.obtainMessage(PROCESS_EXITED, result));
      }
    };
  ...
  private int createSubprocess(final String shell, final String[] env)
      throws IOException {
  ...
  private void onProcessExit(final int result) { onProcessExit(); }
  ...
  /**
   * Send SIGHUP to a process group, SIGHUP notifies a terminal client, that the
   * terminal have been disconnected, and usually results in client's death,
   * unless it's process is a daemon or have been somehow else detached from the
   * terminal (for example, by the "nohup" utility).
   */
