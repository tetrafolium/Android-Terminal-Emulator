...
////////////////////////////////////////////////////////////
public ColorValue(final Context context, final ImageView imgview,
                  final String result[]) {
...
public void colorValue() {
	...
	builder = AlertDialogCompat.newInstanceBuilder(
		context, AlertDialogCompat.THEME_HOLO_DARK);
	...
	lt.setText(context.getString(
			   R.string.colorvalue_label_lock_button_column)); //"LOCK");
	lt.setPadding(lt.getPaddingLeft(), lt.getPaddingTop(), 5,
	              lt.getPaddingBottom());
	...
	vh.addView(value);
	value.setHint(context.getString(
			      R.string.colorvalue_icon_text_entry_hint)); //"Enter icon text");
	lv.addView(vh);
	lv.addView(lt);
				...
				private void doProgressChanged(
					final SeekBar seekBar, final int progress, final boolean fromUser) {
				        if (fromUser && started) {
				                ...
				                int k = (color[0] << 24) | (color[1] << 16) | (color[2] << 8) |
				                        color[3];
							...
							start = end = (Integer)seekBar.getTag();
				                for (int i = start; i <= end; i++) {
				                        if (i == me || (barLock && locks[i])) {
				                                ...
				                                sb[i].setBackgroundColor(0xFF << 24 |
				                                                         (progress << (24 - i * 8)));
	...
	DialogInterface.OnClickListener ocl =
		...
		public void onClick(final DialogInterface dialog, final int which) {
			buttonHit(which, (color[0] << 24) | (color[1] << 16) |
			          (color[2] << 8) | color[3]);
		}
	};
...
//////////////////////////////////////////////////////////////////////
public void toHexWindow(final TextView tv, final int k) {
...
////////////////////////////////////////////////////////////
private void buttonHit(final int hit, final int color) {
	switch (hit) {
	...
	case AlertDialog.BUTTON_POSITIVE: //  OK == set
		...
		if (!imgtext.equals("")) {
			imgview.setImageBitmap(TextIcon.getTextIcon(imgtext, color, 96, 96));
		}
		return;
	}
}
