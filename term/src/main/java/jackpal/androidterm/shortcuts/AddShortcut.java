...
protected void onCreate(final Bundle savedInstanceState) {
	...
	if (action != null &&
	    action.equals("android.intent.action.CREATE_SHORTCUT"))
		makeShortcut();
	else
		finish();
}
...
void makeShortcut() {
	...
	final AlertDialogCompat.Builder alert =
		AlertDialogCompat.newInstanceBuilder(context,
		                                     AlertDialogCompat.THEME_HOLO_DARK);
	...
	if (!path.equals(""))
		et[0].setText(path);
	et[PATH].setHint(
		getString(R.string.addshortcut_command_hint)); //"command");
	et[NAME].setText(name);
	et[ARGS].setHint(
		getString(R.string.addshortcut_example_hint)); //"--example=\"a\"");
	et[ARGS].setOnFocusChangeListener(new OnFocusChangeListener() {
			public void onFocusChange(final View view, final boolean focus) {
			        if (!focus) {
			                String s;
	...
	btn_path.setText(
		getString(R.string.addshortcut_button_find_command)); //"Find command");
		       ...
		       : new File(lastPath).getParentFile();
			        ...
			        if (SP.getBoolean("useInternalScriptFinder", false)) {
			                ...
			                .setClass(getApplicationContext(),
			                          jackpal.androidterm.shortcuts.FSNavigator.class)
			                .setData(Uri.fromFile(get))
			                .putExtra(
						"title",
						getString(
							R.string.addshortcut_navigator_title)); //"SELECT SHORTCUT
			                                                                        //TARGET")
				} else {
	...
	lv.addView(layoutTextViewH(
			   getString(
				   R.string
				   .addshortcut_command_window_instructions) //"Command window
			                                                     //requires full path,
			                                                     //no arguments. For
			                                                     //other commands use
			                                                     //Arguments window
	...
	lv.addView(layoutTextViewH(getString(R.string.addshortcut_arguments_label),
	                           et[ARGS]));
	lv.addView(layoutTextViewH(getString(R.string.addshortcut_shortcut_label),
	                           et[NAME]));
	...
	btn_color.setText(
		getString(R.string.addshortcut_button_text_icon)); //"Text icon");
	...
	lv.addView(layoutTextViewH(
			   getString(
				   R.string.addshortcut_text_icon_instructions) //"Optionally create a
			                                                        //text icon:"
			   ,
			   null, false));
	...
	alert.setPositiveButton(
		android.R.string.yes, new DialogInterface.OnClickListener() {
			public void onClick(final DialogInterface dialog, final int which) {
			        buildShortcut(path, et[ARGS].getText().toString(),
			                      et[NAME].getText().toString(), iconText[1],
			                      (Integer)img.getTag());
			}
		});
...
//////////////////////////////////////////////////////////////////////
LinearLayout layoutTextViewH(final String text, final View vw) {
...
//////////////////////////////////////////////////////////////////////
LinearLayout layoutViewViewH(final View vw1, final View vw2) {
	LinearLayout.LayoutParams lp = new LinearLayout.LayoutParams(
		LinearLayout.LayoutParams.FILL_PARENT,
		LinearLayout.LayoutParams.WRAP_CONTENT, 1);
	LinearLayout lh = new LinearLayout(context);
...
//////////////////////////////////////////////////////////////////////
void buildShortcut(final String path, final String arguments,
                   final String shortcutName, final String shortcutText,
                   final int shortcutColor) {
	...
	if (keys == null) {
		try {
			...
			Log.e(TermDebug.LOG_TAG,
			      "Generating shortcut encryption keys failed: " + e.toString());
	...
	if (shortcutText != null && !shortcutText.equals("")) {
		...
		wrapper.putExtra(
			...
			Intent.ShortcutIconResource.fromContext(
				context, jackpal.androidterm.R.drawable.ic_launcher));
	}
