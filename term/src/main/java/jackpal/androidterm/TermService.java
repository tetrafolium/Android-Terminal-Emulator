...
public class TermService extends Service implements TermSession.FinishCallback {
  ...
  public void onCreate() {
    ...
    Notification notification = new Notification(
        R.drawable.ic_stat_service_notification_icon,
    ...
    notification.setLatestEventInfo(
        this, getText(R.string.application_terminal),
  ...
  public SessionList getSessions() { return mTermSessions; }
  ...
  private final class RBinder extends ITerminal.Stub {
    @Override
    public IntentSender
    startSession(final ParcelFileDescriptor pseudoTerminalMultiplexerFd,
                 final ResultReceiver callback) {
      final String sessionHandle = UUID.randomUUID().toString();

      // distinct Intent Uri and PendingIntent requestCode must be sufficient to
      // avoid collisions
      final Intent switchIntent =
          new Intent(RemoteInterface.PRIVACT_OPEN_NEW_WINDOW)
      ...
      final PendingIntent result = PendingIntent.getActivity(
          getApplicationContext(), sessionHandle.hashCode(), switchIntent, 0);
            ...
            new Handler(Looper.getMainLooper()).post(new Runnable() {
              ...
              public void run() {
                ...
                try {
                  ...
                  session = new BoundSession(pseudoTerminalMultiplexerFd,
                                             settings, niceName);
  ...
  private final class RBinderCleanupCallback
    ...
    public RBinderCleanupCallback(final PendingIntent result,
                                  final ResultReceiver callback) {
      this.result = result;
