...
public UnicodeTranscript(final int columns, final int totalRows,
                         final int screenRows, final int defaultStyle) {
	mColumns = columns;
...
public void setDefaultStyle(final int defaultStyle) {
	mDefaultStyle = defaultStyle;
}
 ...
 */
public boolean resize(final int newColumns, final int newRows,
                      final int[] cursor) {
 ...
 */
private void blockCopyLines(final int src, final int len, final int shift) {
	int totalRows = mTotalRows;
 ...
 */
public void scroll(final int topMargin, final int bottomMargin,
                   final int style) {
 ...
 */
public void blockCopy(final int sx, final int sy, final int w, final int h,
	...
	if (sy > dy) {
		...
		for (int y = 0; y < h; y++) {
			...
			if (lines[srcRow] instanceof char[] && lines[dstRow] instanceof
			    char[]) {
 ...
 */
public void blockSet(final int sx, final int sy, final int w, final int h,
                     final int val, final int style) {
	if (sx < 0 || sx + w > mColumns || sy < 0 || sy + h > mScreenRows) {
		Log.e(TAG, "illegal arguments! " + sx + " " + sy + " " + w + " " + h +
		      " " + val + " " + mColumns + " " + mScreenRows);
		throw new IllegalArgumentException();
	}
...
public static int charWidth(final char cHigh, final char cLow) {
 ...
 * @return The display width of the Unicode code point.
 */
public static int charWidth(final char[] chars, final int index) {
	char c = chars[index];
 ...
 */
public char[] getLine(final int row, final int x1, final int x2) {
 ...
 */
public char[] getLine(final int row) {
	return getLine(row, 0, mColumns, true);
}

private char[] getLine(final int row, final int x1, final int x2,
 ...
 */
public StyleRow getLineColor(final int row, final int x1, final int x2) {
	return getLineColor(row, x1, x2, false);
}

public StyleRow getLineColor(final int row) {
	return getLineColor(row, 0, mColumns, true);
}

private StyleRow getLineColor(final int row, final int x1, final int x2,
                              final boolean strictBounds) {
	if (row < -mActiveTranscriptRows || row > mScreenRows - 1) {
		throw new IllegalArgumentException();
	}
	...
	if (color != null) {
		...
		if (!strictBounds && mLines[row] != null &&
			...
			if (x1 > 0 &&
			    line.findStartOfColumn(x1 - 1) == line.findStartOfColumn(x1)) {
				--x1;
			}
			if (x2 < columns - 1 &&
			    line.findStartOfColumn(x2 + 1) == line.findStartOfColumn(x2)) {
				++x2;
			}
		}
		if (x1 == 0 && x2 == columns) {
...
public boolean getChar(final int row, final int column) {
	return getChar(row, column, 0);
}

public boolean getChar(final int row, final int column, final int charIndex) {
 ...
 * @param offset The offset in the array at which the character will be
 *     placed.
 * @return Whether or not there are characters following this one in the
 *     column.
 */
public boolean getChar(final int row, final int column, final int charIndex,
                       final char[] out, final int offset) {
	if (row < -mActiveTranscriptRows || row > mScreenRows - 1) {
		throw new IllegalArgumentException();
	}
	row = externalToInternalRow(row);
...
private boolean isBasicChar(final int codePoint) {
	return !(charWidth(codePoint) != 1 || Character.charCount(codePoint) != 1);
}

private char[] allocateBasicLine(final int row, final int columns) {
...
private FullUnicodeLine allocateFullLine(final int row, final int columns) {
...
public boolean setChar(final int column, final int row, final int codePoint,
                       final int style) {
...
public boolean setChar(final int column, final int row, final int codePoint) {
	if (row >= mScreenRows || column >= mColumns) {
		Log.e(TAG, "illegal arguments! " + row + " " + column + " " +
		      mScreenRows + " " + mColumns);
		throw new IllegalArgumentException();
	}
	row = externalToInternalRow(row);
...
public FullUnicodeLine(final int columns) {
...
public FullUnicodeLine(final char[] basicLine) {
...
private void commonConstructor(final int columns) {
	mColumns = columns;
...
public boolean getChar(final int column, final int charIndex,
                       final char[] out, final int offset) {
...
public void setChar(final int column, final int codePoint) {
	int columns = mColumns;
	...
	if (oldCharWidth == 2 && charWidth == 1 ||
	    wasExtraColForWideChar && charWidth == 2) {
		...
		if (spaceUsed + 1 > text.length) {
			...
			System.arraycopy(text, 0, newText, 0,
			                 wasExtraColForWideChar ? pos : nextPos);
		}
		...
		if (wasExtraColForWideChar) {
			...
			System.arraycopy(text, nextPos, newText, nextPos + 1,
			                 spaceUsed - nextPos);
			newText[nextPos] = ' ';
		}
	...
	if (oldCharWidth == 1 && charWidth == 2 ||
		...
		if (column == columns - 2) {
			...
			if (nextWidth == 2) {
				...
				if (nextLen > 1) {
					System.arraycopy(text, nextPos + nextLen, text, nextPos + 1,
