...
class TranscriptScreen implements Screen {
   ...
   *        screen.
   */
  public TranscriptScreen(final int columns, final int totalRows,
                          final int screenRows, final ColorScheme scheme) {
    init(columns, totalRows, screenRows, TextStyle.kNormalTextStyle);
  }

  private void init(final int columns, final int totalRows,
                    final int screenRows, final int style) {
    mColumns = columns;
  ...
  public void setLineWrap(final int row) { mData.setLineWrap(row); }
   ...
   */
  public void set(final int x, final int y, final int codePoint,
                  final int style) {
   ...
   * @param selx2 the text selection end X coordinate, if equals to selx1 don't
   *     draw selection
   * @param imeText current IME text, to be rendered at cursor
   * @param cursorMode the cursor mode. See TextRenderer.
   */
  public final void drawText(final int row, final Canvas canvas, final float x,
                             final float y, final TextRenderer renderer,
                             final int cx, final int selx1, final int selx2,
                             final String imeText, final int cursorMode) {
    char[] line;
    StyleRow color;
    int cursorWidth = 1;
    try {
    ...
    if (line == null) {
      ...
      if (selx1 != selx2) {
        ...
        Arrays.fill(blank, ' ');
        renderer.drawTextRun(canvas, x, y, selx1, selx2 - selx1, blank, 0, 1,
                             true, defaultStyle, cx, 0, 1, 1, cursorMode);
      }
      if (cx != -1) {
        char[] blank = new char[1];
        Arrays.fill(blank, ' ');
        // We need to draw the cursor
        renderer.drawTextRun(canvas, x, y, cx, 1, blank, 0, 1, true,
                             defaultStyle, cx, 0, 1, 1, cursorMode);
      }
    ...
    while (column < columns && index < lineLen && line[index] != '\0') {
      ...
      if (style != lastStyle || selectionStyle != lastSelectionStyle ||
          (width > 0 && forceFlushRun)) {
        if (lastRunStart >= 0) {
          renderer.drawTextRun(canvas, x, y, lastRunStart, runWidth, line,
                               lastRunStartIndex, index - lastRunStartIndex,
                               lastSelectionStyle, lastStyle, cx, cursorIndex,
                               cursorIncr, cursorWidth, cursorMode);
        }
        lastStyle = style;
    ...
    if (cx >= 0 && imeText.length() > 0) {
      ...
      renderer.drawTextRun(canvas, x, y, imePosition, imeLength,
                           ...
                           TextStyle.encode(0x0f, 0x00, TextStyle.fxNormal), -1,
                           0, 0, 0, 0);
    }
  }
   ...
   */
  public int getActiveRows() { return mData.getActiveRows(); }
  ...
  public String getTranscriptText() {
    return internalGetTranscriptText(null, 0, -mData.getActiveTranscriptRows(),
                                     mColumns, mScreenRows);
  }

  public String getTranscriptText(final GrowableIntArray colors) {
    return internalGetTranscriptText(
        colors, 0, -mData.getActiveTranscriptRows(), mColumns, mScreenRows);
  }

  public String getSelectedText(final int selX1, final int selY1,
                                final int selX2, final int selY2) {
    return internalGetTranscriptText(null, selX1, selY1, selX2, selY2);
  }

  public String getSelectedText(final GrowableIntArray colors, final int selX1,
                                final int selY1, final int selX2,
                                final int selY2) {
    return internalGetTranscriptText(colors, selX1, selY1, selX2, selY2);
  }

  private String internalGetTranscriptText(final GrowableIntArray colors,
                                           final int selX1, final int selY1,
                                           final int selX2, final int selY2) {
  ...
  public boolean fastResize(final int columns, final int rows,
                            final int[] cursor) {
    if (mData == null) {
   ...
   */
  boolean getScriptLineWrap(final int row) { return mData.getLineWrap(row); }
