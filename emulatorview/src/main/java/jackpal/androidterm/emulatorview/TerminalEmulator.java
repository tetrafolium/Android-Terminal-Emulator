...
class TerminalEmulator {
  public void setKeyListener(final TermKeyListener l) { mKeyListener = l; }
  ...
  static {
    ...
    mSpecialGraphicsCharMap['m'] =
        0x2514; // Lower left hand corner (box drawing)
    mSpecialGraphicsCharMap['j'] =
        0x2518; // Lower right hand corner (box drawing)
    mSpecialGraphicsCharMap['l'] =
        0x250C; // Upper left hand corner (box drawing)
    mSpecialGraphicsCharMap['k'] =
        0x2510; // Upper right hand corner (box drawing)
    ...
    mSpecialGraphicsCharMap['t'] =
        0x251C; // T pointing rightwards (box drawing)
    mSpecialGraphicsCharMap['v'] = 0x2534; // T pointing upwards (box drawing)
    mSpecialGraphicsCharMap['n'] =
        0x253C; // Large plus/lines crossing (box drawing)
   ...
   */
  public TerminalEmulator(final TermSession session,
                          final TranscriptScreen screen, final int columns,
                          final int rows, final ColorScheme scheme) {
    mSession = session;
    mMainBuffer = screen;
    mScreen = mMainBuffer;
  ...
  public TranscriptScreen getScreen() { return mScreen; }
  ...
  public void updateSize(final int columns, final int rows) {
    ...
    if (!fastResize) {
      ...
      charAtCursor = screen.getSelectedText(cursorColor, mCursorCol, mCursorRow,
   ...
   */
  public final int getMouseTrackingMode() { return mMouseTrackingMode; }
  ...
  private void process(final byte b) { process(b, true); }
  ...
  private void continueSequence() { mContinueSequence = true; }
  ...
  private void doEscSelectLeftParen(final byte b) { doSelectCharSet(0, b); }

  private void doEscSelectRightParen(final byte b) { doSelectCharSet(1, b); }
  ...
  private void doEsc(final byte b) {
    ...
    case '8': // DECRC restore cursor
      ...
      mDecFlags =
          (mDecFlags & ~K_DECSC_DECRC_MASK) | mSavedDecFlags_DECSC_DECRC;
      break;
    ...
    case 'M': // Reverse index
      if (mCursorRow <= mTopMargin) {
        mScreen.blockCopy(0, mTopMargin, mColumns,
                          mBottomMargin - (mTopMargin + 1), 0, mTopMargin + 1);
  ...
  private void doEscLeftSquareBracket(final byte b) {
    ...
    case 'n': // Esc [ Pn n - ECMA-48 Status Report Commands
      ...
      case 6: // Cursor position report (CPR):
        ...
        // the cursor location.
        byte[] cpr = String
                         .format(Locale.US, "\033[%d;%dR", mCursorRow + 1,
                                 mCursorCol + 1)
                         .getBytes();
  ...
  private void selectGraphicRendition() {
    ...
    for (int i = 0; i <= mArgIndex; i++) {
      ...
      if (code == 0) { // reset
      ...
      } else if (code == 3) { // italics, but rarely used as such; "standout"
                              // (inverse colors) with TERM=screen
        ...
        setAltCharSet(true);
      } else if (code == 22) { // Normal color or intensity, neither bright,
                               // bold nor faint
        // mEffect &= ~(TextStyle.fxBold | TextStyle.fxFaint);
        mEffect &= ~TextStyle.fxBold;
      } else if (code == 23) { // not italic, but rarely used as such; clears
                               // standout with TERM=screen
        mEffect &= ~TextStyle.fxItalic;
  ...
  private int getForeColor() { return mForeColor; }

  private int getBackColor() { return mBackColor; }

  private int getEffect() { return mEffect; }
  ...
  private void startCollectingOSCArgs() { mOSCArgLength = 0; }
  ...
  private void startTokenizingOSC() { mOSCArgTokenizerIndex = 0; }
  ...
  private void finishSequence() { mEscapeState = ESC_NONE; }
  ...
  private void emit(final int c, final int style) {
    ...
    if ((autoWrap) &&
        (mCursorCol == mColumns - 1 && (mAboutToAutoWrap || width == 2))) {
    ...
    if (mInsertMode & width != 0) { // Move character to right one space
      ...
      if (destCol < mColumns) {
        mScreen.blockCopy(mCursorCol, mCursorRow, mColumns - destCol, 1,
                          destCol, mCursorRow);
      }
    }
  ...
  private void emit(final int c) { emit(c, getStyle()); }
   ...
   *
   * @param c A char[2] containing either a single UTF-16 char or a surrogate
   *     pair to be sent to the screen.
   */
   ...
   */
  private void emit(final char[] c, final int offset, final int length,
                    final int style) {
  ...
  public boolean getUTF8Mode() { return mUTF8Mode; }
  ...
  public String getSelectedText(final int x1, final int y1, final int x2,
                                final int y2) {
