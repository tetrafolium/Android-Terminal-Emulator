...
class TermKeyListener {
  ...
  /**
   * Disabled for now because it interferes with ALT processing on phones with
   * physical keyboards.
   */
  ...
  private void initKeyCodes() {
    ...
    mKeyMap.put(KEYMOD_CTRL | KEYMOD_ALT | KEYMOD_SHIFT | KEYCODE_DPAD_LEFT,
                "\033[1;8D");
    ...
    mKeyMap.put(KEYMOD_CTRL | KEYMOD_ALT | KEYMOD_SHIFT | KEYCODE_DPAD_RIGHT,
                "\033[1;8C");
    ...
    mKeyMap.put(KEYMOD_CTRL | KEYMOD_ALT | KEYMOD_SHIFT | KEYCODE_DPAD_UP,
                "\033[1;8A");
    ...
    mKeyMap.put(KEYMOD_CTRL | KEYMOD_ALT | KEYMOD_SHIFT | KEYCODE_DPAD_DOWN,
                "\033[1;8B");
  ...
  public void setCursorKeysApplicationMode(final boolean val) {
    if (LOG_MISC) {
  ...
  private class ModifierKey {
     ...
     *
     */
    public ModifierKey() { mState = UNPRESSED; }
   ...
   *
   */
  public TermKeyListener(final TermSession termSession) {
    mTermSession = termSession;
    initKeyCodes();
    updateCursorMode();
  }

  public void setBackKeyCharacter(final int code) { mBackKeyCode = code; }

  public void setAltSendsEsc(final boolean flag) { mAltSendsEsc = flag; }

  public void handleHardwareControlKey(final boolean down) {
  ...
  public void handleControlKey(final boolean down) {
    if (down) {
      mControlKey.onPress();
    } else {
  ...
  public void handleFnKey(final boolean down) {
    if (down) {
      mFnKey.onPress();
    } else {
      mFnKey.onRelease();
  ...
  public void setTermType(final String termType) { setFnKeys(termType); }

  private void setFnKeys(final String termType) {
  ...
  public int mapControlChar(final int ch) {
    return mapControlChar(mHardwareControlKey || mControlKey.isActive(),
                          mFnKey.isActive(), ch);
  }

  public int mapControlChar(final boolean control, final boolean fn,
                            final int ch) {
    int result = ch;
    if (control) {
   ...
   *
   */
  public void keyDown(final int keyCode, final KeyEvent event,
                      final boolean appMode, final boolean allowToggle)
      throws IOException {
    if (LOG_KEYS) {
      Log.i(TAG, "keyDown(" + keyCode + "," + event + "," + appMode + "," +
                     allowToggle + ")");
    }
    ...
    default: {
      ...
      if (mCombiningAccent != 0) {
        ...
        if (LOG_COMBINING_ACCENT) {
          Log.i(TAG, "getDeadChar(" + mCombiningAccent + ", " + unaccentedChar +
                         ") -> " + result);
        }
        mCombiningAccent = 0;
      }
  ...
  private static int getCursorModeHelper(final ModifierKey key,
                                         final int shift) {
    return key.getUIMode() << shift;
  }

  static boolean isEventFromToggleDevice(final KeyEvent event) {
  ...
  public boolean handleKeyCode(final int keyCode, final KeyEvent event,
                               final boolean appMode) throws IOException {
    String code = null;
    if (event != null) {
      int keyMod = 0;
   ...
   */
  public void keyUp(final int keyCode, final KeyEvent event) {
